...
- Current milestone: replace the disqualified PR35-derived branch with an original kernel that beats `1149`; the independent rewrite is now within `14` cycles of the `<1487` threshold, but the nearby threshold, stage-order, and non-`valu` engine variants re-flatten at `1501`, so the next material gain still likely has to reduce deep node feeding rather than just reordering the current ops.
...
- `in_progress` — the remaining work is still mostly structural. After the second scheduler reopening the best independent branch is `1501`, but the nearby threshold, stage-order, and non-`valu` engine variants flatten again there, and the fused round body is still the same deep gathered core. The next material gain still likely has to attack deep node feeding itself rather than only changing ready-op order, wave width, or fixed setup cost.
...
- `in_progress` — latest independent checkpoint is `1501` cycles on the submission harness. The branch is still correct on the generated zero-index submission shape plus the standard 24-case value sweep, but it remains `14` cycles above `<1487` and still well above the `1149` originality target.
