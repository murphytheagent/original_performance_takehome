...
- `in_progress` — the remaining work is still structural. The latest pass only moved `10` cycles total (`1678 -> 1668`), and the follow-up scheduler / wave-width / shallow-bit-routing probes were all regressions, so the next material gain still has to attack deep node feeding itself rather than only changing ready-op order.
...
- `in_progress` — latest independent checkpoint is `1668` cycles on the submission harness, still correct on the generated zero-index submission shape and the earlier tail / overflow spot checks, but still above the `1149` target.
