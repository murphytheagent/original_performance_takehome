...
- Current milestone: replace the disqualified PR35-derived branch with an original kernel that beats `1149`; the remaining gap is now smaller, but the latest probes still say cheap branch-bit shortcuts, wider waves, and simple carried-address overlap are not enough. The next step still has to reduce true deep-round node feeding, though the inter-wave stream-overlap line may still have a little residual headroom.
...
- `in_progress` — the remaining work is still structural. After the final-store overlap rewrite the best independent branch is `1537`, but the fused round body is still the same deep gathered core and the latest probes still say that wider waves, simple carried-address recurrence, descending-order final-store overlap, and first-wave in-DAG load rewrites do not pay. The next material gain still has to attack deep node feeding itself rather than only changing ready-op order, wave width, or fixed setup cost.
...
- `in_progress` — latest independent checkpoint is `1537` cycles on the submission harness. The branch is still correct on the generated zero-index submission shape plus the standard 24-case value sweep, but it remains above both the `1487` and `1149` targets.
