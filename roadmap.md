...
- `in_progress` — the remaining work is still structural. After the inter-wave overlap rewrite the best independent branch is `1544`, but the fused round body is still the same deep gathered core and the latest probes still say that wider waves and simple carried-address recurrence do not pay. The next material gain still has to attack deep node feeding itself rather than only changing ready-op order, wave width, or fixed setup cost.
...
- `in_progress` — latest independent checkpoint is `1544` cycles on the submission harness, still correct on the generated zero-index submission shape and the earlier tail / overflow spot checks, but still above the `1149` target.
