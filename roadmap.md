...
- `in_progress` — the remaining work is still structural. The wider scheduler search and a split-phase shallow/deep prototype both failed to beat `1622`, so the next material gain still has to attack deep node feeding itself rather than only changing ready-op order or wave layout.
...
- `in_progress` — latest independent checkpoint is `1622` cycles on the submission harness, still correct on the generated zero-index submission shape and the earlier tail / overflow spot checks, but still above the `1149` target.
