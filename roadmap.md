...
- 2026-03-12 03:52 UTC: The current best independent rewrite branch is now `1508` cycles on the unmodified submission harness. This pass did not touch the algorithm or the tests. A broader exact-shape scheduler search found one more real valu-side overlap win: prioritize `valu` ops with `stage >= 8` before earlier stages, and within that later band drain lower-numbered groups first while the earlier-stage band keeps the old higher-numbered-group preference. The same 24-case value sweep still matched `reference_kernel2`.
- Current milestone: replace the disqualified PR35-derived branch with an original kernel that beats `1149`; the independent rewrite is now back within `21` cycles of the `<1487` threshold, but the nearby two-band/three-band scheduler variants flatten at `1508`, so the next material gain still likely has to reduce deep node feeding rather than just reordering the current ops.
...
- `in_progress` — the remaining work is still mostly structural. After the new scheduler split the best independent branch is `1508`, but nearby two-band and three-band scheduler neighborhoods flatten there, and the fused round body is still the same deep gathered core. The next material gain still likely has to attack deep node feeding itself rather than only changing ready-op order, wave width, or fixed setup cost.
...
- `in_progress` — latest independent checkpoint is `1508` cycles on the submission harness. The branch is still correct on the generated zero-index submission shape plus the standard 24-case value sweep, but it remains `21` cycles above `<1487` and still well above the `1149` originality target.
