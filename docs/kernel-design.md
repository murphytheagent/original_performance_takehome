...
- 2026-03-11 19:45 UTC: current best verified result on the independent rewrite branch is `1668` cycles.
...
- There is now about `51` words of scratch headroom on the live `1668` branch (`1485 / 1536` used). That was enough to keep one extra depth-3 cache temp but not enough to recover the old `18`-group wave width or to stage a second full per-group helper vector.
